			...
			<div class="card mb-4">
				...
				<div class="card-body">
					...
					<div class="d-flex justify-content-end">
						<a href="<?php echo site_url('propietarios/agregar') ?>" class="btn btn-success text-white">
					...
					<div class="example">
						...
						<div class="tab-content rounded-bottom">
							<div class="tab-pane p-3 active preview" role="tabpanel" id="preview-1001">
								<table class="table table-bordered table-hover" id='propietarios_table'>
									<thead>
										<tr>
											<th scope="col">#</th>
					  				...
					  				<tbody>
					  					<?php if($propietarios): ?>
					  					<?php $i=1; ?>
					  					<?php foreach($propietarios as $key=>$data): ?>
										<tr class="table-info">
											<th scope="row"><?php echo $i ?></th>
											<td><?php echo $data->nombres ?></td>
											<td><?php echo $data->apellido_paterno ?></td>
											<td><?php echo $data->apellido_materno ?></td>
											<td><?php echo $data->numero_celular_1 ?></td>
											<td><?php echo $data->numero_celular_2 ?></td>
											...
											<td>
												<ul class="list-group list-group-horizontal">
													<li class="list-group-item" style="border: none !important;background-color: transparent!important;">
														<a class="btn btn-warning" href='<?php echo site_url("propietarios/editar/$data->id") ?>'>
													...
													<li class="list-group-item" style="border: none !important;background-color: transparent!important;">
														<a data-id='<?php echo $data->id ?>' class="btn btn-danger" href="#" data-coreui-toggle="modal" data-coreui-target="#modalEliminar">
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button class="btn btn-danger text-white" type="button" id="btnConfirmarEliminar">Sí, eliminar</button>
			</div>
		</div>
	</div>
</div>
...
new DataTable('#propietarios_table', {
	language: {
...
// Cuando el modal se va a abrir, captura el id del elemento que lo disparó
$('#modalEliminar').on('show.coreui.modal', function (event) {
	var idRegistro = $(event.relatedTarget).data('id');
$('#btnConfirmarEliminar').data('id', idRegistro);
});

// Al confirmar, ejecuta la petición al controlador
$('#btnConfirmarEliminar').on('click', function () {
	var idRegistro = $(this).data('id');

	$.ajax({
		url: "<?php echo site_url('propietarios/eliminar/') ?>" + idRegistro, // Ajusta la ruta a tu proyecto
		type: 'POST', // o 'DELETE' si tu controlador lo soporta
		data: {
			id: idRegistro,
		},
		success: function (response) {
			$('#modalEliminar').modal('hide');
			// Aquí puedes recargar la tabla, mostrar alerta, etc.
			location.reload();
		},
		error: function (xhr) {
